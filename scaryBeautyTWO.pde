...
void setup()
{
  ...
  getTunes = new SoundFile(this,"londonRoadsThree.wav");
...
void makeCircle()
{
  ...
  //
  count += spectrum[musiCount%spectrum.length];
  ...
  for(int i = 0; i < numOfParticles+1; i++)
  {
    ...
    for(int j = 0; j < numOfParticles+1; j++)
    {
      ...
      radiusOne = superShape(lon,5,.4+count,.4+count,.4+count);
...
void rotateCircle()
{
  ...
  {
    rotateX(rotateRate+=.003);//+=.004+count*.0001);
    rotateZ(rotateRate);
  }
